...
if [[ ! -z "${INPUT_SUB_FETCH_DEPTH}" ]]; then
	...
	git submodule update --init --depth --no-single-branch "${INPUT_SUB_FETCH_DEPTH}" "${INPUT_PATH}" || error "__Line:${LINENO}__Error: Could not checkout submodule hash referenced by ${PR_BRANCH} (is it pushed to remote?)"
else
